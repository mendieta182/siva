        ...
        <nav aria-label="Table navigation">      
          <ul class="inline-flex items-center">
              <Link type="button" v-if="data.current_page > 1" 
                class="px-1.5 py-1 ml-0.5 border rounded-md  focus:outline-none focus:shadow-outline-blue" aria-label="Previous" :href="data.first_page_url">
              ...
              <Link type="button" v-if="data.current_page > 1"
                class="px-1.5 py-1 ml-0.5 border rounded-md  focus:outline-none focus:shadow-outline-blue" aria-label="Previous" :href="data.prev_page_url">
            ...
            <li v-for="(link, k) in data.links.slice(1,-1)">
              <Link
                type="button"                
                :key="k"
                class="px-2.5 py-1 ml-0.5 border rounded-md  focus:outline-none focus:shadow-outline-blue"
            ...
            <Link v-if="data.current_page<data.last_page" :href="data.next_page_url" type="button" 
              class="px-1.5 ml-0.5 border py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-blue" aria-label="Next">
              <svg class="w-4 h-4 fill-current" aria-hidden="true" viewBox="0 0 20 20">
                <path d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" fill-rule="evenodd"></path>
              </svg>
            </Link>
            <Link type="button" v-if="data.current_page<data.last_page" 
              class="px-1.5 ml-0.5 border py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-blue" aria-label="Last Page"
